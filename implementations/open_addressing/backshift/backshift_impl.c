/**
 * @file    backshift_impl.c
 * @brief   Open-addressing backend with backward-shift deletion.
 *
 * Implements a linear-probing table that fills removal gaps by shifting later
 * entries backwards, avoiding tombstone buildup during steady-state workloads.
 *
 ...
 */
static void backshift_destroy_impl(void *impl);
 ...
 */
static ht_result backshift_insert_impl(void *impl, ht_key_t key,
                                       ht_val_t value);
 ...
 *         `HT_ERR_NOT_FOUND`.
 */
static ht_result backshift_get_impl(const void *impl, ht_key_t key,
                                    ht_val_t *value_out);
 ...
 *         `HT_ERR_NOT_FOUND`.
 */
static ht_result backshift_remove_impl(void *impl, ht_key_t key);
 ...
 */
static size_t backshift_size_impl(const void *impl);
 ...
 */
static size_t backshift_capacity_impl(const void *impl);
 ...
 */
static double backshift_load_factor_impl(const void *impl);
 ...
 */
static ht_result backshift_reset_stats_impl(void *impl);
 ...
 */
static ht_result backshift_resize(backshift_table *t, size_t new_capacity);
 ...
 */
static ht_result backshift_insert_rehash(backshift_table *t, uint64_t hash,
...
/**
 * @brief Find the slot to use for an insert.
 *
 * @param t Table to search.
 * @param key Key to insert.
 * @param hash Cached hash for the key.
 ...
 *         `HT_ERR_FULL`.
 */
static ht_result backshift_find_insert_slot(const backshift_table *t,
...
static const struct ht_vtable BACKSHIFT_VTABLE = {
    .destroy = backshift_destroy_impl,
    .insert = backshift_insert_impl,
    .get = backshift_get_impl,
    .remove = backshift_remove_impl,
    .size = backshift_size_impl,
    .capacity = backshift_capacity_impl,
    .load_factor = backshift_load_factor_impl,
    .reserve = backshift_reserve_impl,
    .rehash = backshift_rehash_impl,
    .get_stats = backshift_get_stats_impl,
    .reset_stats = backshift_reset_stats_impl,
    .bind_bench_iface = backshift_bind_bench_iface};
...
int backshift_bind_bench_iface(void *ctx, bench_iface *out) {
  ...
  out->ctx = ctx;
  out->insert = backshift_insert_impl;
  out->get = backshift_get_impl;
  out->remove = backshift_remove_impl;
...
retry:
  rc = backshift_find_insert_slot(t, key, hash, &slot, &found_existing,
                                  &probe_len);
  if (rc != HT_OK) {
  ...
  /* Backshift deletion keeps used == size, so only a truly empty slot
  ...
  t->slots[slot].state = BACKSHIFT_SLOT_FULL;
...
static ht_result backshift_get_impl(const void *impl, ht_key_t key,
  ...
  if (t->collect_stats) {
    ((backshift_table *)t)->stats.lookups++;
  }

  hash = t->hash_fn(key, t->hash_seed);
  rc = backshift_find_slot(t, key, hash, &slot, &probe_len);

  if (rc != HT_OK) {
    if (t->collect_stats) {
      ((backshift_table *)t)->stats.lookup_misses++;
      HT_UPDATE_PROBE_STATS((backshift_table *)t, probe_len);
    }
...
static ht_result backshift_resize(backshift_table *t, size_t new_capacity) {
  backshift_slot *old_slots;
  size_t old_capacity;
  size_t old_size;
  backshift_slot *new_slots;
  ...
  for (i = 0; i < old_capacity; i++) {
    if (old_slots[i].state == BACKSHIFT_SLOT_FULL) {
      rc = backshift_insert_rehash(t, old_slots[i].hash, old_slots[i].key,
                                   old_slots[i].value);
      if (rc != HT_OK) {
        free(t->slots);
        t->slots = old_slots;
...

        {
          ...
          for (j = 0; j < old_capacity; j++) {
            if (old_slots[j].state != BACKSHIFT_SLOT_EMPTY) {
              used++;
            }
          }
          t->used = used;
...
static ht_result backshift_insert_rehash(backshift_table *t, uint64_t hash,
  ...
  for (i = 0; i < t->capacity; i++) {
    ...
    if (t->slots[idx].state != BACKSHIFT_SLOT_FULL) {
      if (t->slots[idx].state == BACKSHIFT_SLOT_EMPTY) {
        t->used++;
      }

      t->slots[idx].state = BACKSHIFT_SLOT_FULL;
...
static ht_result backshift_find_insert_slot(const backshift_table *t,
  ...
  for (i = 0; i < t->capacity; i++) {
    ...
    if (t->slots[idx].state == BACKSHIFT_SLOT_FULL) {
      if (t->slots[idx].hash == hash && t->slots[idx].key == key) {
        *slot_out = idx;
        if (found_existing != NULL) {
          *found_existing = 1;
        }
